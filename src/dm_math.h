...
DM_API DM_INLINE
void dm_vec2_scale_inpl(dm_vec2 vec, float scale, dm_vec2* out)
{
	*out = dm_vec2_scale(vec, scale);
}
DM_API DM_INLINE
...
float dm_vec2_angle(dm_vec2 left, dm_vec2 right)
{
	...
	return (float)acosf(
		DM_CLAMP(
			dm_vec2_dot(left, right) / (llen * rlen),
			-1.0f, 1.0f
		)
	);
}
...
DM_API DM_INLINE
void dm_vec3_add_scalar_inpl(dm_vec3 vec, float scalar, dm_vec3* out)
{
	*out = dm_vec3_add_scalar(vec, scalar);
}
DM_API DM_INLINE
...
DM_API DM_INLINE
void dm_vec3_sub_scalar_inpl(dm_vec3 vec, float scalar, dm_vec3* out)
{
	*out = dm_vec3_sub_scalar(vec, scalar);
}
DM_API DM_INLINE
...
DM_API DM_INLINE
void dm_vec3_scale_inpl(dm_vec3 vec, float scale, dm_vec3* out)
{
	*out = dm_vec3_scale(vec, scale);
}
DM_API DM_INLINE
...
{
	float len = (float)sqrt(dm_vec3_dot(vec, vec));
	//if (len == 0) DM_WARN("Vec3 has length 0!");
	return len;
}
DM_API DM_INLINE
...
float dm_vec3_angle(dm_vec3 left, dm_vec3 right)
{
	...
	return (float)acosf(
		DM_CLAMP(
			dm_vec3_dot(left, right) / (llen * rlen),
			-1.0f, 1.0f
		)
	);
}
...
DM_API DM_INLINE
void dm_vec4_add_scalar_inpl(dm_vec4 vec, float scalar, dm_vec4* out)
{
	*out = dm_vec4_add_scalar(vec, scalar);
}
DM_API DM_INLINE
...
DM_API DM_INLINE
void dm_vec4_sub_scalar_inpl(dm_vec4 vec, float scalar, dm_vec4* out)
{
	*out = dm_vec4_sub_scalar(vec, scalar);
}
DM_API DM_INLINE
...
DM_API DM_INLINE
void dm_vec4_scale_inpl(dm_vec4 vec, float scale, dm_vec4* out)
{
	*out = dm_vec4_scale(vec, scale);
}
DM_API DM_INLINE
...
float dm_vec4_angle(dm_vec4 left, dm_vec4 right)
{
	...
	return (float)acosf(
		DM_CLAMP(
			dm_vec4_dot(left, right) / (llen * rlen),
			-1.0f, 1.0f
		)
	);
}
