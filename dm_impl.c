...
void dm_vec4_norm(const float vec[N4], float out[N4])
{
    ...
    dm_mm_float sum  = dm_mm_hadd_ps(v_sq, v_sq);
    sum  = dm_mm_hadd_ps(sum, sum);
    
    dm_mm_float sqrt = dm_mm_rsqrt_ps(sum);
    
    v = dm_mm_mul_ps(v, sqrt);
...
void dm_mat4_transpose(const float mat[M4], float out[M4])
{
    ...
    dm_mm_float row2 = dm_mm_load_ps(mat + 4);
    dm_mm_float row3 = dm_mm_load_ps(mat + 8);
    dm_mm_float row4 = dm_mm_load_ps(mat + 12);
    
    dm_mm_transpose_mat4(&row1, &row2, &row3, &row4);
    
    dm_mm_store_ps(out + 0,  row1);
    dm_mm_store_ps(out + 4,  row2);
    dm_mm_store_ps(out + 8,  row3);
    dm_mm_store_ps(out + 12, row4);
}
...
void dm_mat4_mul_mat4(float left[M4], float right[M4], float out[M4])
{
    ...
    left_r  = dm_mm_load_ps(left + 4);
    right_r = dm_mm_load_ps(right_t + 4);
    row_2   = dm_mm_mul_ps(left_r, right_r);
    
    left_r  = dm_mm_load_ps(left + 8);
    right_r = dm_mm_load_ps(right_t + 8);
    row_3   = dm_mm_mul_ps(left_r, right_r);
    
    left_r  = dm_mm_load_ps(left + 12);
    right_r = dm_mm_load_ps(right_t + 12);
    row_4   = dm_mm_mul_ps(left_r, right_r);
    
    dm_mm_store_ps(out + 0,  row_1);
    dm_mm_store_ps(out + 4,  row_2);
    dm_mm_store_ps(out + 8,  row_3);
    dm_mm_store_ps(out + 12, row_4);
}
...
void dm_mat4_mul_vec4(const float mat[M4], const float vec[N4], float out[N4])
{
    ...
    const dm_mm_float m2 = dm_mm_load_ps(mat_t + 4);
    const dm_mm_float m3 = dm_mm_load_ps(mat_t + 8);
    const dm_mm_float m4 = dm_mm_load_ps(mat_t + 12);
...
{
    float new_vec[N4] = { vec[0], vec[1], vec[2], val_4 };
...
void dm_mat4_mul_scalar(float mat[M4], float scalar, float out[M4])
{
    ...
    row_2 = dm_mm_load_ps(mat + 4);
    row_2 = dm_mm_mul_ps(row_2, s);
    
    row_3 = dm_mm_load_ps(mat + 8);
    row_3 = dm_mm_mul_ps(row_3, s);
    
    row_4 = dm_mm_load_ps(mat + 12);
...
void dm_mat4_add_mat4(float left[M4], float right[M4], float out[M4])
{
    ...
    l     = dm_mm_load_ps(left + 4);
    r     = dm_mm_load_ps(right + 4);
    row_2 = dm_mm_add_ps(l, r);
    
    l     = dm_mm_load_ps(left + 8);
    r     = dm_mm_load_ps(right + 8);
    row_3 = dm_mm_add_ps(l, r);
    
    l     = dm_mm_load_ps(left + 12);
    r     = dm_mm_load_ps(right + 12);
