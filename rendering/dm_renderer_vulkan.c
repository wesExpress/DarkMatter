...
{
    DM_VULKAN_SAMPLER_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER,
...
void dm_renderer_backend_shutdown(dm_context* context)
{
    ...
    {
        dm_vulkan_raster_pipeline* pipeline = &vulkan_renderer->raster_pipes[i];
    ...
    {
        dm_vulkan_compute_pipeline* pipeline = &vulkan_renderer->compute_pipes[i];
...
bool dm_renderer_backend_create_constant_buffer(dm_constant_buffer_desc desc, dm_resource_handle* handle, dm_renderer* renderer)
{
    ...
    for(uint8_t i=0; i<DM_VULKAN_MAX_FRAMES_IN_FLIGHT; i++)
    {
        ...
        // descriptors
#ifdef DM_VULKAN_DESCRIPTOR_BUFFERS
        dm_vulkan_copy_buffer_descriptor(host_buffer, VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER, 0, desc.size, i, vulkan_renderer);
#else
...
bool dm_renderer_backend_create_storage_buffer(dm_storage_buffer_desc desc, dm_resource_handle* handle, dm_renderer* renderer)
{
    ...
    for(uint8_t i=0; i<DM_VULKAN_MAX_FRAMES_IN_FLIGHT; i++)
    {
        ...
        // descriptors
#ifdef DM_VULKAN_DESCRIPTOR_BUFFERS
        dm_vulkan_copy_buffer_descriptor(device_buffer, VK_DESCRIPTOR_TYPE_STORAGE_BUFFER, 1, desc.size, i, vulkan_renderer);
#else
        VkDescriptorBufferInfo buffer_info = { 0 };
...
bool dm_renderer_backend_create_texture(dm_texture_desc desc, dm_resource_handle* handle, dm_renderer* renderer)
{
    ...
    for(uint8_t i=0; i<DM_VULKAN_MAX_FRAMES_IN_FLIGHT; i++)
    {
        ...
        vkCmdCopyBufferToImage(cmd_buffer, staging_buffer->buffer, image->image, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, 1, &copy);
    ...
    vr = vkCreateSampler(vulkan_renderer->device.logical, &sampler_create_info, NULL, &texture.sampler);
    if(!dm_vulkan_decode_vr(vr))
    {
    ...
    dm_vulkan_descriptor_data descriptor_data = descriptor_buffer->descriptor_data[0];
    ...
    for(uint8_t i=0; i<DM_VULKAN_MAX_FRAMES_IN_FLIGHT; i++)
    {
        ...
        write_set.dstBinding      = DM_VULKAN_SAMPLER_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER;
...
#ifdef DM_VULKAN_DESCRIPTOR_BUFFERS
    handle->descriptor_index = descriptor_buffer->descriptor_data[0].count++;
#else
    handle->descriptor_index = handle->index;
#endif
...
#ifdef DM_VULKAN_DESCRIPTOR_BUFFERS
    properties.vulkan_1_3.pNext             = &properties.descriptor_buffer;
    properties.descriptor_buffer.sType      = VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_DESCRIPTOR_BUFFER_PROPERTIES_EXT;
    properties.descriptor_buffer.pNext      = &properties.acceleration_structure;
#else
...
#ifdef DM_VULKAN_DESCRIPTOR_BUFFERS
    features.vulkan_1_3.pNext             = &features.descriptor_buffer;
    features.descriptor_buffer.sType      = VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_DESCRIPTOR_BUFFER_FEATURES_EXT;
    features.descriptor_buffer.pNext      = &features.acceleration_structure;
#else
...
#ifdef DM_VULKAN_DESCRIPTOR_BUFFERS
    vulkan_renderer->device.properties.vulkan_1_3.pNext             = &vulkan_renderer->device.properties.descriptor_buffer;
    vulkan_renderer->device.properties.descriptor_buffer.pNext      = &vulkan_renderer->device.properties.acceleration_structure;
#else
...
#ifdef DM_VULKAN_DESCRIPTOR_BUFFERS
    vulkan_renderer->device.features.vulkan_1_3.pNext             = &vulkan_renderer->device.features.descriptor_buffer;
    vulkan_renderer->device.features.descriptor_buffer.pNext      = &vulkan_renderer->device.features.acceleration_structure;
#else
...
bool dm_vulkan_create_bindless_layout(dm_vulkan_renderer* vulkan_renderer)
{
    ...
    sampler_descriptor_types[DM_VULKAN_SAMPLER_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER] = VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER;
...
bool dm_vulkan_create_descriptor_buffers(dm_vulkan_renderer* vulkan_renderer)
{
    ...
    sampler_data[DM_VULKAN_SAMPLER_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER].size = vulkan_renderer->device.properties.descriptor_buffer.combinedImageSamplerDescriptorSize;
...
bool dm_vulkan_create_bindless_descriptor_pool(dm_vulkan_renderer* vulkan_renderer)
{
    ...
    sampler_pool_size[DM_VULKAN_SAMPLER_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER].type            = VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER;
    sampler_pool_size[DM_VULKAN_SAMPLER_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER].descriptorCount = DM_VULKAN_MAX_DESCRIPTORS;
